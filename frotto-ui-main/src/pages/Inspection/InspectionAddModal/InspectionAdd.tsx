...
const InspectionAdd: React.FC<InspectionAddModalProps> = ({
  closeModal,
  initialValues,
  carId,
}) => {
  ...
  const [activeCarBodyDamage, setActiveCarBodyDamage] =
    useState<CarBodyDamageModel>({});
  ...
  useEffect(() => {
    if (formInitial.id === undefined) {
      loadActiveBodyDamages();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const carDamagesList = useMemo(() => {
    if (watch("carBodyDamages")) {
      return watch("carBodyDamages");
    }
    return [];
  }, [watch]);
  ...
  const closeBodyDamageModal = useCallback(
    (newCarDamage: CarBodyDamageModel) => {
      ...
      if (newCarDamage) {
        ...
        if (indexFound === -1) {
          finalcarBodyDamageArray.push(newCarDamage);
        } else {
          finalcarBodyDamageArray[indexFound] = newCarDamage;
        }
  ...
  const onSubmit = async (newInspectionForm: InspectionForm) => {
    ...
    try {
      ...
      if (newInspection.id) {
        const urlPatch = endpoints.INSPECTIONS_EDIT({
          pathVariables: {
            id: newInspection.id,
          },
        });
        ...
        const urlPost = endpoints.INSPECTIONS({
          pathVariables: {
            id: carId,
          },
        });
  ...
  const onDelete = async () => {
    ...
    try {
      await api.delete(
        endpoints.INSPECTIONS_EDIT({ pathVariables: { id: formInitial.id } })
      );
      setisLoading(false);
      closeModal({});
    } catch (e) {
  ...
  return (
    <IonPage id="car-inspection-add-page">
            ...
            {watch("expenses").map(
              (expense: InspectionExpenseModel, index) => {
                if (expense) {
                  ...
                  return <ItemNotFound text={TEXT.noExpenses} />;
                }
              }
            )}
            {watch("expenses") && watch("expenses").length === 0 && (
              <ItemNotFound text={TEXT.noExpenses} />
            )}
          </IonList>
          <IonList>
            <IonListHeader>
            ...
            {carDamagesList.map((carBodyDamage: CarBodyDamageModel, index) => {
              if (carBodyDamage) {
                ...
                return <ItemNotFound text={TEXT.noCarBodyDamages} />;
              }
            })}
            {carDamagesList.length === 0 && (
              <ItemNotFound text={TEXT.noCarBodyDamages} />
            )}
          </IonList>
          {formInitial.id && (
            <FormDeleteButton
      ...
      <IonModal isOpen={isBodyDamageModalOpen} backdropDismiss={false}>
        <BodyDamageAdd
          closeModal={closeBodyDamageModal}
          carId={carId}
          initialValues={activeCarBodyDamage}
        />
      </IonModal>
    </IonPage>
  );
