import {
  IonButton, IonButtons, IonCardContent, IonContent, IonHeader,
...
const MyPlanPage: React.FC = () => {
  ...
  if (loading) return <IonPage id="my-plan-page"><PageHeader /><IonContent><div className="app-shell app-shell--compact"><Loading /></div></IonContent></IonPage>;
  if (error || !billing) return <IonPage id="my-plan-page"><PageHeader /><IonContent><div className="app-shell app-shell--compact"><ItemNotFound title="Não foi possível carregar seu plano" description={error || "Entre novamente para consultar seus dados."} actionLabel="Tentar novamente" onAction={() => void load()} /></div></IonContent></IonPage>;
  ...
  return <IonPage id="my-plan-page">
    <PageHeader />
    <IonContent>
      <div className="app-shell app-shell--compact my-plan-shell">
        ...
        <FrottoCard className="my-plan-current">
          <IonCardContent>
            <div className="my-plan-current__header"><div><span className="my-plan-eyebrow">Seu plano</span><h1>{friendlyPlan(billing.planCode)}</h1><p>{sourceDetail(billing)}</p></div><FrottoBadge variant="neutral">{sourceLabel(billing.subscriptionSource)}</FrottoBadge></div>
            {confirmed && <div className="my-plan-alert" role="status"><FrottoBadge variant="success">Cancelamento agendado</FrottoBadge><span>{endDate ? `Seu plano ficará ativo até ${endDate}. Não haverá nova renovação.` : "Seu plano ficará ativo até o fim do período atual. Não haverá nova renovação."}</span></div>}
            ...
            <div className="my-plan-facts"><div><span>Preço atual</span><strong>{currentPrice}</strong></div><div><span>Status</span><FrottoBadge variant={statusVariant(billing.subscriptionStatus)}>{statusLabel(billing.subscriptionStatus)}</FrottoBadge></div><div><span>Uso da frota</span><strong>{fleetUsage(billing)}</strong></div></div>
        ...
        <section ref={plansRef} className="my-plan-section"><div className="my-plan-heading"><span className="my-plan-eyebrow">Compare</span><h2>Planos disponíveis</h2><p>Escolha uma estrutura que acompanhe o tamanho da sua frota.</p></div><div className="my-plan-grid">
          {plans.map((plan) => { const current = plan.code === billing.planCode; const recommended = plan.code === billing.requiredPlanCode; const compatible = isPlanCompatible(plan, billing.activeVehicleCount); return <FrottoCard key={plan.code} className={`my-plan-plan${recommended ? " my-plan-plan--recommended" : ""}`}><IonCardContent>
            <div className="my-plan-plan__badges">{current && <FrottoBadge variant="info">Seu plano atual</FrottoBadge>}{recommended && <FrottoBadge variant="success">Recomendado para sua frota</FrottoBadge>}</div><h3>{PLAN_LABELS[plan.code]}</h3><p className="my-plan-range">{vehicleRange(plan)}</p><div className="my-plan-price">{plan.billingModel === "PROGRESSIVE" && <small>Base </small>}<strong>{money(plan.monthlyBasePrice)}</strong><small>/mês</small></div><p>{plan.billingModel === "PROGRESSIVE" ? "Cobrança progressiva conforme a frota" : "Valor mensal fixo"}</p>
    ...
    <IonModal isOpen={cancelModalOpen} canDismiss={!cancelLoading} backdropDismiss={!cancelLoading} onDidDismiss={() => { if (!cancelInFlight.current) setCancelModalOpen(false); }} className="my-plan-modal">
      <FrottoModal
        pageId="my-plan-cancel-modal"
        title="Cancelar assinatura"
        onCancel={() => setCancelModalOpen(false)}
        cancelLabel="Manter assinatura"
        cancelDisabled={cancelLoading}
        primaryLabel={cancelLoading ? "Cancelando..." : "Confirmar cancelamento"}
        onPrimaryAction={() => void cancelSubscription()}
        primaryDisabled={cancelLoading}
        isLoading={cancelLoading}
      >
        <div className="my-plan-modal__body">
