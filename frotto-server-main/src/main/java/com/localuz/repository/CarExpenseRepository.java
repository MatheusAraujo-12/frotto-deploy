...
public interface CarExpenseRepository extends JpaRepository<CarExpense, Long> {
    ...
    @Query(
        "select carExpense from CarExpense carExpense join carExpense.car car where car.user.login = ?#{principal.username} ORDER BY carExpense.date DESC"
    )
    List<CarExpense> findByCurrentUser();
}
